...
std::vector<std::shared_ptr<Object>> TileMapParser::Parse(const std::string& FilePath, const Vector2& Offset)
{
    ...
    {
        for (const auto& TileProperties : *TileLayer.second)
        {            
            ...
            auto TransformComp = TileObject->GetComponent<TransformComponent>();
...
std::shared_ptr<TileSheetData> TileMapParser::BuildTileSheetData(xml_node<>* RootNode)
{
    ...
    xml_node<>* TileSheetLocation = RootNode->first_node("tileset");
    ...
    TileSheetDataObj.TileSize.x = static_cast<float>(std::atoi(TileSheetNode->first_attribute("tilewidth")->value()));
    TileSheetDataObj.TileSize.y = static_cast<float>(std::atoi(TileSheetNode->first_attribute("tileheight")->value()));
    ...
    return std::make_shared<TileSheetData>(TileSheetDataObj);
}

std::shared_ptr<MapTiles> TileMapParser::BuildMapTiles(xml_node<>* RootNode)
{
    std::shared_ptr<TileSheetData> TileSheetDataObj = BuildTileSheetData(RootNode);
...
std::pair<std::string, std::shared_ptr<Layer>> TileMapParser::BuildLayer(xml_node<>* LayerNode,
                                                                         const std::shared_ptr<TileSheetData>&
                                                                         TileSheet)
{
    TileSet TileSetData;
        ...
        if (TileId != 0)
        {
            ...
            if (Iter == TileSetData.end())
            {
                Vector2 TexturePosition{
                    static_cast<float>((TileId - 1) % TileSheet->Rows),
            ...
            LayerData->emplace_back(TileData);
        }
